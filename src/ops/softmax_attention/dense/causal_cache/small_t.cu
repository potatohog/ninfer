...
void launch_tc_partial_bf16(const Tensor& q, CacheInput input, const Tensor& pos, float scale,
    ...
    causal_attention_small_t_tc_partial_bf16_kernel<Geometry, TokenTile, WarpsPerCta, MultiBatch,
        ...
        <<<grid, kBlock, 0, stream>>>(
            ...
            invocation.tail_rows, slot_stride, scale, static_cast<__nv_bfloat16*>(partial_acc.data),
...
void launch_tc_partial_tail(const Tensor& q, CacheInput input, const Tensor& pos, float scale,
    ...
    causal_attention_small_t_tc_partial_tail_kernel<Geometry, TokenTile, WarpsPerCta, MultiBatch,
                                                    Masked, CacheInput>
        <<<grid, kBlock, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(q.data), input,
            ...
            static_cast<__nv_bfloat16*>(partial_acc.data) + acc_offset,
    ...
    do {                                                                                           \
        ...
        if (tail_enabled) {                                                                        \
            const auto launch_tail = [&]<bool MultiBatch, bool Masked>() {                         \
                launch_tc_partial_tail<Geometry, (TOKENS), (WARPS), MultiBatch, Masked>(           \
